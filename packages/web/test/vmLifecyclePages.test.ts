...
function installCreatePageFetch(createStatus: number, createBody: unknown): FetchCounts {
  const counts: FetchCounts = {}
  ...
  globalThis.fetch = ((input: string | URL | Request, init?: RequestInit) => {
    ...
    const body = createPageReferenceData(path)
    if (!body) {
      return Promise.resolve(
        new Response(
          JSON.stringify({
...
function createPageReferenceData(path: string): unknown {
  switch (path) {
...
async function submitCreateForm(renderer: ReactTestRenderer): Promise<void> {
